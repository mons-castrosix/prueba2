...
def listar_inventario(request):
    #PRODUCTOS GENERAL
    productos=BodegaProductos.objects.all().select_related('obra').values('categoria','unidad','descripcion').annotate(total=Sum('cantidad')).order_by('descripcion')
   #PRODUCTOS POR OBRA
    productos2=BodegaProductos.objects.all().select_related('obra').values('clave','categoria','bodega__nombre','unidad','descripcion').annotate(total=Sum('cantidad')).order_by('descripcion')
